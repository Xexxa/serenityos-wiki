...
Presently, you can resize the entire FontEditor form to your liking. You can turn-on or turn-off the visibility of Font Metadata and Unicode Blocks. You can change the glyph editor window zoom factor.  All of these things are currently available in FontEditor automatically.
...
Figure above shows where you can find FontEditor from inside the SerenityOS desktop. It is joined together with other similarly amazing applications like GML Playground, Hack Studio, Hex Editor, Inspector, Keyboard Mapper, Profiler and Theme Editor. Each of the apps by themselves are worthy of their own self-contained tutorial which we hope to soon see materialize. These apps are all native to SerenityOS as they are for SerenityOS, by SerenityOS and of SerenityOS. Python was included simply because this is the `Development` section of Serenity menu. Python is part of the ` Ports ` package and is actively being managed and maintained by [@linusg](https://github.com/linusg).
...
Figure above depicts a search session on [fonts.serenityos.net](https://fonts.serenityos.net) portal.
